...
- **서버사이드 페이지네이션, 정렬 & 필터링**: 자산 목록 조회 시 서버사이드 페이지네이션(페이지당 20건), 검색/카테고리 필터, 컬럼(자산명·자산번호·구매일·구매가·상태) 클릭 정렬을 지원하여 대량 데이터에서도 원하는 자산을 빠르게 찾을 수 있습니다.
- **엑셀 내보내기 / 일괄 등록(Import)**: 현재 검색·필터·정렬 조건이 반영된 자산 목록을 엑셀로 내려받을 수 있고, 같은 컬럼 형식의 엑셀 파일을 업로드해 여러 자산을 한 번에 등록할 수 있습니다. 행 단위로 검증하여 실패한 행(중복 자산번호, 형식 오류 등)은 사유와 함께 건너뛰고 나머지는 정상 반영합니다.
- **자산 변경 이력 기록 (Audit Log)**: 자산 및 유지보수 이력의 생성(CREATE), 수정(UPDATE), 삭제(DELETE)가 발생하면, 어떤 필드가 어떻게 바뀌었는지(이전 값/이후 값)와 수정자, 일시 등의 변경 로그를 `asset_audit_log` 테이블에 자동으로 기록합니다. 자산 상세 페이지의 "변경 이력" 섹션에서 자산별로 확인할 수 있고, 관리자는 "감사 로그" 메뉴에서 시스템 전체의 변경 이력을 한 화면에서 조회·필터링할 수 있습니다.
...
- **일괄 업로드/일괄 적용**: 여러 개의 견적서/영수증 파일을 한 번에 업로드하고, 분석이 끝난 항목들을 모아서 한 번에 적용할 수 있습니다.
- **유지보수 기록 즉시 적용 및 적용 취소**: 분석 및 파싱 완료된 내용에서 "적용" 버튼을 누르면 실제 해당 자산의 유지보수 이력 기록으로 자동 등록되며, 잘못 적용한 경우 "적용 취소"로 방금 등록된 유지보수 기록을 되돌릴 수 있습니다.

### 5. 자연어 검색 (Natural Language Search)
- "5년 이상 사용한 노트북 보여줘", "A동에 있는 IT 장비 중 교체 필요한 것" 등의 한국어 자연어 질문을 입력하면, LLM이 이를 카테고리/위치/사용기간/상태 필터 조건으로 파싱하여 검색합니다.
...
- **AI 엔드포인트 요청 제한 (Rate Limiting)**: gn-cab API 키 자체의 분당 호출 제한을 넘지 않도록, AI/자연어검색/보고서 엔드포인트에는 서버단에서 분당 호출 횟수를 제한해 여러 사용자가 동시에 AI 버튼을 연타해도 안정적으로 동작합니다.
...
- **운영 안정성**: 배포 서버는 uvicorn 워커 2개로 동작하고, 매일 새벽 DB를 자동 백업(14일 보관)하며, LLM 호출 실패는 조용히 무시되지 않고 서버 로그에 기록됩니다. GitHub Actions로 `backend-py` 변경 시마다 pytest가 자동 실행되어 회귀를 방지합니다.
...
│   │   ├── pages/           # 개별 화면 (대시보드, 자산 관리/상세, 예산 관리, 파일 분석,
│   │   │                    #   감사 로그 등. React.lazy로 코드 스플리팅)
...
- `GET /api/assets/audit-logs` - 시스템 전체 변경 이력(감사 로그) 조회, 작업 유형/작업자 필터 지원 (관리자 전용)
- `POST /api/assets/import` - 엑셀 파일로 자산 일괄 등록 (행 단위 검증, 실패 행은 사유와 함께 건너뜀, 관리자 전용)
...
- `POST /api/files/batch-apply` - 여러 파일을 한 번에 적용
- `POST /api/files/{id}/unapply` - 적용된 유지보수 기록 적용 취소(되돌리기)
...
> **보안 안전장치**: `.env`는 `.gitignore`에 등록되어 git에 커밋되지 않습니다 (`git add -A`를 써도 자동으로 제외됨). 배포 서버에서는 파일 권한을 `chmod 600`으로 설정해 소유자(root) 외에는 읽을 수 없도록 제한하는 것을 권장합니다.
