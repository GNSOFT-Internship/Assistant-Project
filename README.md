...
- **고장/정비 이력 검색 추가**: "전원고장이 1번이라도 있었던 장비", "하드디스크 수리 이력이 있는 노트북" 등의 질문도 고장 키워드(`failureKeyword`) 및 최소 발생 횟수(`minFailureCount`) 조건으로 정확히 파싱하여, 자산의 유지보수 기록(`maintenance_record`) 및 조치 내용까지 추적 및 매칭하여 검색 결과를 제공합니다.
...
│   │   ├── llm.py           # 사내 Qwen3.5 / Gemini LLM 연동 클라이언트
...
DEMO_MODE=true
# 사내 GPU 서버 API Key (AI 기능 실구동 시 필수 입력)
GN_API_KEY=
GN_MODEL=qwen35
```
