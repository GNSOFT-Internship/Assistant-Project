...
- **고장 유형 분포 시각화 개선**: 조각이 많아질 때 가독성이 떨어지던 파이 차트를 개선하여 상위 5개 주요 고장 유형은 가로 막대 차트로 제공하고, 전체 고장 유형 목록은 건수 및 비율 순으로 정렬된 표 형식으로 제공합니다. 고장 유형 종류가 많은 경우를 대비해 10개씩 페이지네이션 처리되어 있습니다. (단순 점검/정기점검용 "없음" 유형은 통계 분석에서 자동 제외)
...
### 6. AI 교체 우선순위 추천
- 사용기간(내용연수 대비, 30점) · 누적 수리 비용(구매가 대비 수리 비율, 25점) · 고장 발생 빈도(10점) · **카테고리별 업무 중요도(20점)** · 교체 필요 상태 여부(15점, 상태가 "교체 필요"면 가산)를 종합적으로 점수화(0~100점)하여 교체 대상 순위를 추천합니다.
- **카테고리별 업무 중요도 반영**: `category`는 자유 문자열이라 고정 목록이 없어서, 새 카테고리(장비 종류)가 자산 등록 시 처음 등장하면 LLM에게 "고장 시 업무 영향도"를 0~100점으로 한 번만 물어보고 `category_importance` 테이블에 캐싱합니다(같은 카테고리 자산이 몇 개든 재사용, AI 미설정/실패 시 중립값 50점으로 폴백). 그 결과 NAS(핵심 인프라)처럼 업무 중요도가 높은 장비가 정수기처럼 대체가 쉬운 장비보다 같은 사용기간·수리비여도 더 높은 우선순위로 추천됩니다. 관리자는 교체 추천 화면의 "카테고리 중요도 관리" 패널에서 이 값을 언제든 직접 덮어쓸 수 있고, 이 경우 이후 AI가 자동으로 재계산하지 않습니다.
- 가용 예산을 입력하면 예산 한도 내에서 가장 시급히 교체해야 할 최적의 자산 조합을 선별하고, 추천 사유를 LLM이 한국어로 요약 서술해 줍니다.
- **추천 사유 캐싱**: 추천 사유 텍스트의 근거가 되는 수치(사용기간·수리비율·고장횟수·카테고리 중요도·점수)를 해시로 저장해두고, 페이지를 다시 열어도 근거 수치가 지난번과 동일하면 AI를 다시 호출하지 않고 저장된 문구를 그대로 재사용합니다. 추천된 자산 중 하나라도 근거 수치가 바뀌면(새 유지보수 이력, 카테고리 중요도 재조정 등) 전체 배치를 다시 생성해, "총평"이 일부만 갱신되어 앞뒤가 안 맞는 상황을 방지합니다.
...
- **운영 안정성**: 배포 서버는 uvicorn 단일 워커로 동작합니다(LLM 응답 캐시 등 프로세스 메모리 기반 상태가 워커별로 나뉘지 않도록 하기 위함이며, 2인용 내부 도구 규모에서는 스레드풀만으로 동시 요청 처리에 충분합니다). 메모리가 넉넉하지 않은 서버라 systemd `MemoryHigh`/`MemoryMax`로 안전장치를 걸어뒀고, 매일 새벽 DB를 자동 백업(14일 보관)하며, LLM 호출 실패는 조용히 무시되지 않고 서버 로그에 스택트레이스까지 기록됩니다. **머지 전 PR 단계 CI**: `main`으로 향하는 PR이 열리거나 갱신될 때마다 `ci.yml`이 바뀐 쪽(`backend-py` 변경 시 pytest, `frontend` 변경 시 vitest)만 골라 실행해, 테스트가 깨진 채로 머지되어 `main`이 배포 불가 상태가 되는 것을 미리 막습니다. **머지 후 배포**: `main`에 push되면 별도의 `deploy.yml` 워크플로가 pytest/vitest를 한 번 더 통과시킨 뒤(로컬 pre-push 훅 우회 등에 대비한 이중 안전장치), 서버에 SSH로 접속해 저장소 루트의 `deploy.sh`(git pull → 변경분 재시작/재빌드 → 배포 후 검증)를 자동으로 실행하므로, 로컬에서 서버에 수동으로 SSH 접속해 배포하지 않아도 됩니다.
...
├── .github/workflows/        # ci.yml(main으로 향하는 PR에서 변경분 pytest/vitest 실행),
│                             #   deploy.yml(main push 시 테스트를 한 번 더 통과시킨 뒤 서버 자동 배포)
