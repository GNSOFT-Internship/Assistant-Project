...
#
# 주의: 이 캐시는 프로세스 메모리에만 존재한다. uvicorn을 --workers 2 이상으로
# 띄우면 워커마다 별도 프로세스라 캐시가 서로 공유되지 않아 효과가 워커 수만큼
# 줄어든다. 그래서 프로덕션은 의도적으로 단일 워커(uvicorn 기본값)로 운영 중이다
# (2인용 내부 도구 규모에서는 스레드풀만으로도 동시 요청 처리에 충분하다).
# 나중에 워커를 늘려야 한다면 이 캐시를 Redis 등 프로세스 밖 저장소로 옮겨야 한다.
# ---------------------------------------------------------------------------
