...
def test_fallback_notebook_keyword(client, admin_headers):
    asset = client.post("/api/assets", json=IT_ASSET, headers=admin_headers).json()["data"]
    data = _ask(client, admin_headers, "노트북 몇 대 있어?")
    assert "IT 장비는 총" in data["answer"]
    assert data["hasFilter"] is True
    assert any(a["id"] == asset["id"] for a in data["assets"])
...
def test_fallback_replacement_keyword(client, admin_headers):
    asset = client.post("/api/assets", json=REPLACEMENT_NEEDED_ASSET, headers=admin_headers).json()["data"]
...
def test_price_filter_is_recomputed_by_code_even_if_llm_picks_wrong_assets(client, admin_headers, monkeypatch):
    ...
    cheap = client.post("/api/assets", json={**IT_ASSET, "purchasePrice": 500000}, headers=admin_headers).json()["data"]
    expensive_a = client.post("/api/assets", json={**IT_ASSET, "purchasePrice": 1610000}, headers=admin_headers).json()["data"]
    expensive_b = client.post("/api/assets", json={**IT_ASSET, "purchasePrice": 1450000}, headers=admin_headers).json()["data"]
...
def test_category_field_ignored_when_it_does_not_match_any_real_category(client, admin_headers, monkeypatch):
    ...
    laptop = client.post(
        "/api/assets",
        json={**IT_ASSET, "purchaseDate": "2018-01-01"},
...
def test_keyword_and_used_years_are_recomputed_by_code_not_left_to_llm_judgment(client, admin_headers, monkeypatch):
    ...
    old_laptop = client.post(
        "/api/assets",
        json={**IT_ASSET, "assetName": "노트북 LG Gram", "purchaseDate": "2018-01-01"},
        headers=admin_headers,
    ).json()["data"]
    new_laptop = client.post(
        "/api/assets",
        json={**IT_ASSET, "assetName": "노트북 삼성 갤럭시북", "purchaseDate": "2025-06-01"},
        headers=admin_headers,
    ).json()["data"]
    old_server = client.post(
        "/api/assets",
        json={**IT_ASSET, "assetName": "서버 Dell PowerEdge", "purchaseDate": "2018-01-01"},
        headers=admin_headers,
    ).json()["data"]
...
def test_price_filter_combined_with_category_excludes_other_categories(client, admin_headers, monkeypatch):
    ...
    cheap_it = client.post("/api/assets", json={**IT_ASSET, "purchasePrice": 500000, "category": "IT 장비"}, headers=admin_headers).json()["data"]
    cheap_office = client.post("/api/assets", json={**IT_ASSET, "purchasePrice": 400000, "category": "사무기기"}, headers=admin_headers).json()["data"]
    ...
    data = _ask(client, admin_headers, "100만원 이하인 IT 장비 찾기")
    ids = {a["id"] for a in data["assets"]}
    assert ids == {cheap_it["id"]}
    assert cheap_office["id"] not in ids
