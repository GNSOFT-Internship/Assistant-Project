...
def test_add_update_delete_maintenance_record_and_history(client, admin_headers):
    ...
    assert client.get(f"/api/assets/{asset_id}/maintenance", headers=admin_headers).json()["data"] == []

    history = client.get(f"/api/assets/{asset_id}/history", headers=admin_headers).json()["data"]
