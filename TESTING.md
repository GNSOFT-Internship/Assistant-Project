...
| 구분 | 위치 | 실행 명령 | 최근 결과 |
|---|---|---|---|
| 백엔드 (pytest) | `backend-py/tests/` | `cd backend-py && python -m pytest tests/ -q` | **125개 전체 통과** |
| 프론트엔드 (vitest) | `frontend/src/**/*.test.jsx` | `cd frontend && npx vitest run` | **88개 전체 통과** |
...
### 프론트엔드 테스트 파일 (16개)
`AiAssistant`, `AssetDetail`, `Assets`, `AuditLog`, `Budget`, `Dashboard`, `FileUpload`, `Login`,
